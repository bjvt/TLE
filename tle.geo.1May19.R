...
geo_json <- fromJSON(file = "show.json")
satcat.df <- as.data.frame(geo_json, check.names = TRUE)
...
lla_df <- merge.data.frame(lla_df, sat_master, by="ID")

# reorder the columns for easier reading
lla_df <- lla_df[c(1,5,8,2,3,4)]
