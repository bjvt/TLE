...
# download Three Line Full Catalog (maybe GEO, MEO, LEO) from https://www.space-track.org/#recent
# take the 3 line element file and make two files, 1st one is Satellite Name, 2nd is the modified TLE ready for R
# use the GNU SED Stream Editor
# sed '/^1/d' 3le.2Oct19.txt | sed '/^2/d' | sed 's/^.\{2\}//' > one.txt
# sed '/^0/d' 3le.2Oct19.txt | paste - - | tr -d '\r' | tr -d '\t' > two.txt
...
# read the file with the names - fwf = fixed width format
...
# this is the entire 3LE represented in an R data frame
