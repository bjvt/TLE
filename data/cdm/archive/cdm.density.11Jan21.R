...
# calculate the volumes per bin, create a new dataframe, not that 'counts' need to add 1
df_volume <- data.frame(breaks = c(hist[["breaks"]]), counts = c(hist[["counts"]],1),
                        mids = c(hist[["mids"]],1), alt.density = c(hist[["density"]],1))
