...
At band 550 to 560, the 734 objects has an average of 0.0000196643679 objects per cubic kilometer, one object per  50,853 cubic kilometers.  the 550 to 560 band has about 37,326,397 cubic kilometers.  this is assuming that we are dealing with perfect circles and circular orbits.
...
df_merge <- merge(cdm_public, gp_leo, by.x = "SAT_1_ID", by.y = "NORAD_CAT_ID", all = TRUE)
#df_merge <- merge(gp_leo, cdm_public, by.x = "NORAD_CAT_ID", by.y = "SAT_1_ID", all.y = all)

#changed bins from altitude
...
ghist3 <- ggplot(df_merge, aes(x=Bins)) +
...
```{r density.covariance, echo=FALSE, warning=FALSE}
