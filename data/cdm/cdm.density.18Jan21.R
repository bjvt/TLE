...
# remove some columns, RStudio doesn'ts how extenve # of colums
...
ghist1 <- ggplot(gp_leo, aes(x=altitude)) +
...
ghist2 <- ggplot(gp_leo, aes(x=altitude, fill = OBJECT_TYPE)) +
...
ghist <- ggplot(df_merge, aes(x=Bins)) +
  geom_histogram(binwidth = 20, color="green", aes(y=..density..)) +
  geom_line(stat = "density") +
labs(title="CDM  Probability of LDO Collision Per Band of Orbit",
